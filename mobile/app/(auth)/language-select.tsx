// mobile/app/(auth)/language-select.tsx
...
export default function LanguageSelectScreen() {
  ...
  // ✅ Jouer le message de bienvenue une fois que l'utilisateur est connecté
  useEffect(() => {
    if (user && !welcomePlayed) {
      voiceService.welcome(user.username || 'utilisateur', user.current_streak || 0)
        .catch((err) => {
          const errorMessage = err instanceof Error ? err.message : 'Erreur vocale inconnue';
          console.error('[Voice] Erreur bienvenue:', errorMessage);
        });
  ...
  async function handleStart() {
    ...
    setLoading(true);
    try {
      await selectLanguageRequest(selected);
      await refreshUser();
      router.replace('/(tabs)');
    } catch (error) {
      console.error('[LanguageSelect] Erreur:', error);
    } finally {
      setLoading(false);
    }
  }
