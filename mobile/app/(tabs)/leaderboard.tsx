...
import ErrorMessage from '../../components/common/ErrorMessage'; // ✅ Ajout

export default function LeaderboardScreen() {
  const { theme } = useTheme();
  const [leaderboard, setLeaderboard] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null); // ✅ Ajout
  ...
  useFocusEffect(
    useCallback(() => {
      setLoading(true);
      setError(null); // ✅ Reinitialise l'erreur a chaque nouvelle tentative
      fadeAnim.setValue(0);
      client.get('/leaderboard').then((res: any) => {
        ...
        setError('Impossible de charger le classement. Verifie ta connexion.'); // ✅ Ajout
        setLoading(false);
      });
    }, [])
  );
  ...
  // ✅ Ajout — affiche un message clair si la requete a echoue
  if (error) {
    return (
...
const styles = StyleSheet.create({
  ...
  // ✅ Ajout — conteneur pour le message d'erreur
  errorWrap: { flex: 1, justifyContent: 'center', paddingHorizontal: 20 },
