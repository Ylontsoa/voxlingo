  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]} edges={['top']}>
      <View style={styles.header}>
        ...
        {/* Regroupement à droite : icône Messages (discrète) + pastille de langue */}
        <View style={styles.headerRight}>
          {/* ✅ Icône Messages discrète — pas de fond coloré, contour fin, gris secondaire */}
          <TouchableOpacity
            onPress={() => router.push('/conversations-list')}
            style={styles.messagesIconButton}
            activeOpacity={0.7}
          >
            <FontAwesome name="comment-o" size={20} color={theme.textSecondary} />
          </TouchableOpacity>
      ...
      {/* Traducteur en direct via code a 6 chiffres (inchangé) */}
      <TouchableOpacity onPress={() => router.push('/translator-lobby')} activeOpacity={0.85} style={styles.translatorWrap}>
        <LinearGradient colors={['#6366F1', '#8B5CF6']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.translatorCard}>
          <View style={styles.translatorIconWrap}>
            <FontAwesome name="exchange" size={15} color="#6366F1" />
          </View>
          <View style={styles.translatorTextWrap}>
            <Text style={styles.translatorTitle}>Parler avec quelqu'un</Text>
            <Text style={styles.translatorSubtitle}>Discute en direct, traduit automatiquement</Text>
          </View>
          <FontAwesome name="chevron-right" size={14} color="rgba(255,255,255,0.85)" />
        </LinearGradient>
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  // Conteneur pour aligner icône Messages + pastille de langue
  headerRight: { flexDirection: 'row', alignItems: 'center', gap: 8 },

  // ✅ Icône Messages discrète dans le header — pas de fond, gris secondaire
  messagesIconButton: {
    width: 34, height: 34,
  ...
  // Style de la carte "Parler avec quelqu'un"
  translatorWrap: { marginHorizontal: 20, marginBottom: 12, borderRadius: 18 },
  translatorCard: { flexDirection: 'row', alignItems: 'center', gap: 12, borderRadius: 18, padding: 14 },
  translatorIconWrap: { width: 34, height: 34, borderRadius: 17, backgroundColor: '#ffffff', alignItems: 'center', justifyContent: 'center' },
  translatorTextWrap: { flex: 1 },
  translatorTitle: { color: '#ffffff', fontSize: 14, fontWeight: '800' },
  translatorSubtitle: { color: 'rgba(255,255,255,0.9)', fontSize: 12, marginTop: 1 },
